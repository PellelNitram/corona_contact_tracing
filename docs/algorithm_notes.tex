...
\begin{itemize}
	...
	\begin{itemize}
		\item The adjacency matrix $A$ is time dependent, $A^{(l)}$, and inferred from data. In our use case, $A_{ij} \propto 1/dist(v_i, v_j)$, hence $A_{ij}$ is large when persons $i$ and $j$ have been in \todo{Think about that!}contact.
